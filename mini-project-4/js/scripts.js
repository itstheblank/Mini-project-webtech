...
function showContent(sectionId) {
  ...
  const staticSections = ['courseInfo', 'info', 'web-tech', 'student-info'];
  ...
  if (hardcodedSection && staticSections.includes(sectionId)) {
    ...
    if (sectionId === 'courseInfo') {
      ...
      <a class="w3-bar-item w3-button w3-hover-black" href="#layout-config">Cấu hình Layout</a>
      `;
    ...
    if (items.length > 0) {
      ...
      document.getElementById('dynamic-content-area').innerHTML = '<h3 class="w3-center w3-text-grey" style="margin-top: 50px;">Chưa có menu trái nào được tạo. Hãy vào Admin Page để thêm.</h3>';
    }
...
function renderDynamicLeftMenuContent(leftMenuId) {
  ...
  if (items.length === 0) {
    html += '<h3 class="w3-center w3-text-grey" style="margin-top: 50px; grid-column: span 12;">Chưa có nội dung layout nào được tạo.</h3>';
  }
    ...
    htmlContent = `
      <div class="wiki-search-container">
        <div style="text-align: center; margin-bottom: 20px;">
          ...
          <input type="text" class="wikiSearchTerm w3-input w3-border w3-round" placeholder="Nhập từ khóa tìm kiếm..." autocomplete="off">
        </div>
